var TreeView = React.createClass({
  ...
  propTypes: {
    ...
    unCheckedIcon: React.PropTypes.string,
  ...
  onChildSelected: function (props) {
    this.setState({
      node: props
    })
  },
    ...
    if (data) {
      ...
      data.forEach(function (node) {
        children.push(React.createElement(TreeNode, {
          ...
          visible: true,
          options: _this.props,
          recreate: _this.onChildSelected
        }));
      });
    }
...
var TreeNode = React.createClass({
  ...
  getInitialState: function () {
    ...
    return {
      ...
      iconClass: this.props.options.unCheckedIcon
    }
  },
  iconClicked: function (node, event) {
    if (this.state.iconClass === this.props.options.unCheckedIcon) {
      this.setState(() => ({
        iconClass: this.props.options.checkIcon
      }));
    } else {
      this.setState(() => ({
        iconClass: this.props.options.unCheckedIcon
  ...
  toggleSelected: function (node, event) {
    node.highlightSelected = true;
    this.setState((prevState) => ({
      selected: !prevState.selected,
    }));
    this.props.recreate(this.props);
    //  event.stopPropagation();
  },
  ...
  render: function () {
    ...
    if (!this.props.visible) {
      ...
      if (node.highlightSelected && this.state.selected && node.highlightSelected) {
        style = {
    ...
    var nodeIcon = (
      React.createElement("span", {
        ...
        onClick: this.iconClicked.bind(this, node)
      })
    );
